...
from ctu.coco_image_slicer import CocoImageSlicer
...
class CocoAbsoluteToRelative:
    ...
    def convert_coord_from_orig_to_pad_addition(
      ...
      Dimension Change:
        ...
        if ((original_rel_x is None) and (original_rel_y is None)):
            raise Exception("Both the `original_rel_x` and `original_rel_y` can't be None")
    ...
    def convert_coord_from_pad_to_orig(
        ...
        if ((padded_rel_x is None) and (padded_rel_y is None)):
            raise Exception("Both the `padded_rel_x` and `padded_rel_y` can't be None")
        pady, padx = rel_padding_ht_wd
            ...
            for poly in annotation_info["segmentation"]:
                ...
                for i, e in enumerate(poly):
                    if offset == "pad_to_orig":
                        val = p2o(
                            padded_rel_x=(e if self._is_x_coord(i) else None),
                            padded_rel_y=(None if self._is_x_coord(i) else e),
                            rel_padding_ht_wd=rel_padding_ht_wd
                        )
                    else:
                        val = o2p(
                            original_rel_x=(e if self._is_x_coord(i) else None),
                            original_rel_y=(None if self._is_x_coord(i) else e),
                            rel_padding_ht_wd=rel_padding_ht_wd
                        )
                    new_poly.append(val)
